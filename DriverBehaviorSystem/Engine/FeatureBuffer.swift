...
// MARK: - Baseline
// Kalibrasyon aşamasında kişiye özel hesaplanan referans değerler.
// İki aşamalı kalibrasyon:
//   Faz 1 → EAR baseline (göz aralığı, sessiz bakış)
//   Faz 2 → MAR baseline (ağız aralığı, metin okuma sırasında)
struct Baseline: Codable {
    ...
    /// Göz kapalı eşiği: EAR < baseline * 0.75
    ...
    /// Yawn eşiği: MAR > baseline_konusma * 1.5
    /// Faz 2'de kişi metin okurken MAR baseline alındığı için
    /// konuşma/şarkı normal sayılır, esnemede eşik aşılır.
    /// Kaynak: Abtahi M. et al. (2014). YawDD dataset.
    /// Proceedings of the 5th ACM Multimedia Systems Conference.
    /// → Konuşma MAR'ının 1.5 katı esnemede güvenilir eşik.
    var marYawnThreshold: Double { max(mar * 1.5, 0.20) }
...
// 51 feature — train.py feature_columns ile AYNI SIRA.
//
// Kaynak: Soukupova & Cech (2016) — EAR/MAR hesaplama formülleri.
// Kaynak: Abtahi M. et al. (2014) — YawDD state machine.
...
class FeatureBuffer {
    ...
    // Yawn state machine
    // Minimum süre: 600ms (eski 800ms — daha hassas)
    // Cooldown: 3s — aynı yawn'u çift saymama
    // Kaynak: Abtahi M. et al. (2014). YawDD.
    ...
    func reset() {
        ...
        yawnFrameCount=0; isYawActive=false
        yawnCount=0; yawnCooldownFrames=0
    }
    ...
    func push(ear: Double, mar: Double,
        ...
        // Göz kapalı: EAR ratio < 0.75 [Soukupova & Cech 2016]
        ...
        // Yawn state machine [Abtahi 2014]
        // Faz 2 kalibrasyonu sayesinde konuşma MAR'ı baseline olduğundan
        // normal konuşma/şarkı yawn eşiğini aşmaz
        let yawnMinFrames = Int(fps * 0.6)  // 600ms
        ...
        if mar > baseline.marYawnThreshold {
            ...
            if yawnFrameCount >= yawnMinFrames && !isYawActive {
                ...
                if yawnCooldownFrames == 0 {
                    ...
                    print("🥱 Yawn! #\(yawnCount) MAR=\(String(format:"%.3f",mar)) eşik=\(String(format:"%.3f",baseline.marYawnThreshold))")
                }
            }
        } else {
            yawnFrameCount = 0
    ...
    // MARK: - 51 Feature Vektörü (sequence_config.json ile AYNI SIRA)
    ...
    var blinkRate5s     : Double { rSum(blinkStarts, max(1, Int(fps * 5))) / 5.0 }
